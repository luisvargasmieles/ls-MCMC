function [alpha, rho2] = SAPG_deb_TV(varargin)
%
% [alpha, rho] = SAPG_deb_TV(varargin)
% 
...
% deblurring experiment (See Algorithm 3.1 in paper for details)
%
%%  =========== inputs ====================
%
% 'true_img'                    : true image
...
thrho2_init=sigma^2;
min_thrho2=1e-3*thrho2_init;
max_thrho2=50*thrho2_init;
...
while (stopCriTV >= tol_tv || stopCriRho2 >= tol_rho2) && i < maxSamples
    ...
    if i > 2e2
        TVavg(i) = ((i-2e2-1)/(i-2e2))*TVavg(i-1) + (1/(i-2e2))*thTV(i);
        rho2avg(i) = ((i-2e2-1)/(i-2e2))*rho2avg(i-1) + (1/(i-2e2))*thrho2(i);
